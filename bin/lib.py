...
def archive_dataset_schemas(step_name, local_dict, global_dict):
    ...
    data_sets = filter(lambda x: type(x[1]) == pandas.DataFrame, env_variables.items())  # filter down to Pandas dfs
    ...
    agg_schema_df = pandas.concat(schema_agg)  # aggregate schema list into one df
